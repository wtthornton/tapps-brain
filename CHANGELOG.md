...
- **Tenancy migration plan fixes (TAP-7279 follow-up)** — four corrections found reviewing the live dry run against the recorded disposition, before any `--apply`: (1) R1/R2 now resolve the target *agent* on every re-home instead of carrying a literal placeholder (`'default'`/`'unknown'`) across — order is the `audit_log` actor, then the nlt-ideas-scout ingest-window `'ingest'` provenance, then `'legacy-unattributed'`; (2) a re-home group whose target identity equals its source identity is refused (`SelfCollisionError`) rather than silently applied; (3) rows under the literal `'default'` project are rescued only by an R1 audit-log origin — a mapped `source_agent` (R2) or the ingest key shape (R3) no longer rescues them; (4) rows under a hex-id probe tenant (`project_id` matching `^[0-9a-f]{8,}$`) are excluded from the population entirely, in both tables, and reported in the plan's new `skipped_probe_tenants` section — their disposition is fenced to a separate tenant-hygiene ticket.

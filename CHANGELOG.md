...
- **Tenancy migration under-predicted intra-plan collisions** ([#315](https://github.com/wtthornton/tapps-brain/pull/315)) — the 2026-09-10 tenancy migration plan predicted 10 identity collisions; applying it resolved 49. `_detect_collisions` now checks in-scope rows against each other (not only the live table), and `apply_plan` refuses to run when its own prediction undercounts what it is about to do.
...
- **`/v1/forget` left the hive copy of a memory recallable** ([#314](https://github.com/wtthornton/tapps-brain/pull/314)) — forgetting a memory only archived its private row; a hive-propagated copy stayed reachable via Hive search. `brain_forget` now also archives the hive copy across every namespace this agent's recall would search.
...
- CI: widen the test collection guard to every `tests/` subdirectory ([#310](https://github.com/wtthornton/tapps-brain/pull/310))

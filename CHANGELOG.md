...
### Fixed
- `PostgresConnectionManager.project_context()` / `agent_context()` / `admin_context()` / `namespace_context()` switched from `SET LOCAL` (transaction-scoped) to `SET` (session-scoped) so the bound identity survives multiple transactions inside one pool borrow (TAP-514). Earlier semantics let a caller `commit()` mid-`with` block and silently lose RLS context — fail-closed policies then hid every row, looking like an empty tenant. The pool now registers a `reset` callback that runs `RESET app.project_id; RESET app.agent_id; RESET app.is_admin; RESET tapps.current_namespace` when the connection is returned, so identity cannot leak across borrows; a failed reset closes the connection rather than recycling it. Integration test `tests/integration/test_session_context_persistence.py` covers both the intra-borrow survival and cross-borrow isolation guarantees.
- structured propagation outcomes on `hive_propagate` and `hive_push`: every response now carries a canonical `decision` code (`propagated` | `refused_private_tier` | `refused_client_scope` | `refused_group_not_member`) plus `rule_applied`, `requested_scope`, `effective_scope`, `tier`, and — on refusals — a `would_require` hint (e.g. `{"force": true}` or `{"join_group": "<name>"}`). `hive_push`'s per-entry `pushed` / `skipped` records carry the same taxonomy. Consumers should read the outcome instead of mirroring the brain's `HiveConfig` client-side. The legacy top-level `propagated: bool` and human-readable `reason` string remain for backward compatibility.

...
- **Persistence:** `MemoryPersistence` (and Hive / federation stores) use their own locks around connection use. SQLite is opened in **WAL** mode where configured so readers and writers can overlap at the engine level. By default one writer connection is guarded by ``MemoryPersistence``’s primary lock; optionally (**``TAPPS_SQLITE_MEMORY_READONLY_SEARCH``**) FTS search and sqlite-vec KNN use a **read-only** second connection under a separate lock so those queries need not wait on the writer lock. Hive / federation remain single-connection unless changed later.
...
- **Scaling posture:** Throughput targets are **modest concurrent sessions**, not high-QPS multi-tenant service. If lock or `database is locked` errors appear in production, first tune **SQLite busy timeout** / workload separation; **queue or service extraction** is a product/architecture decision (see **EPIC-050**, **EPIC-051** [`ADR-004`](../planning/adr/ADR-004-scale-single-node-sqlite-defer-service-extraction.md), and `open-issues-roadmap.md` row 22), not a quick config tweak.
- **Performance review backlog (dense retrieval + saves):** Optional future work — packed int8/float16 embeddings vs JSON floats, batch reindex, shorter lock holds around embedding, sqlite-vec format alignment — is summarized in [`docs/guides/embedding-model-card.md`](../guides/embedding-model-card.md) § *Performance review backlog*. Use that table when scheduling perf or schema follow-ups.

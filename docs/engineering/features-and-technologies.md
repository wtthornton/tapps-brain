...
| **Composite ranking** | Weighted score blend | `retrieval.py`: relevance 40%, confidence 30%, recency 15%, frequency 15% via the default profile (code fallback: frequency 10% + graph centrality 5%, TAP-734); per-source trust multipliers after composite; profile can tune scoring where wired. |

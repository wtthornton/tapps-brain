...
|------|------|-------------|---------|
| `embedding` | `list[float] \| None` |  | *required* |
...
```python
def save(self, *, key: str, value: str, namespace: str = 'universal', source_agent: str = 'unknown', tier: str = 'pattern', confidence: float = 0.6, source: str = 'agent', tags: list[str] | None = None, valid_at: str | None = None, invalid_at: str | None = None, superseded_by: str | None = None, conflict_policy: Any = 'supersede', memory_group: str | None = None, embedding: list[float] | None = None) -> dict[str, Any] | None:
```
...
per-entry :meth:[`save`](#save) loop when a backend does not implement it, so
...
# `src.tapps_brain._store_feedback`
...
### `FeedbackMixin`(_MemoryStoreBase)

Flywheel + explicit-feedback API methods (TAP-2833).

**Methods:**

#### `zero_result_gap_signals`
...
```python
def process_feedback(self, *, since: str | None = None, config: Any = None) -> dict[str, Any]:
```
...
```python
def knowledge_gaps(self, limit: int = 10, *, semantic: bool = False) -> list[Any]:
```
...
Last report from `[`generate_report`](#generate-report)` (None if never run).
...
```python
def rate_recall(self, entry_key: str, *, rating: str = 'helpful', session_id: str | None = None, details: dict[str, Any] | None = None) -> FeedbackEvent:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `entry_key` | `str` | The memory entry key that was recalled. | *required* |
| `rating` | `str` | Quality rating — ``"helpful"`` (1.0), ``"partial"`` (0.5), ``"irrelevant"`` (0.0), or ``"outdated"`` (0.0). | 'helpful' |
| `session_id` | `str \| None` | Optional calling session identifier. | None |
| `details` | `dict[str, Any] \| None` | Optional additional metadata. | None |

**Returns:** `FeedbackEvent` - The persisted ``FeedbackEvent``.
...
```python
def report_gap(self, query: str, *, session_id: str | None = None, details: dict[str, Any] | None = None) -> FeedbackEvent:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `query` | `str` | The query or topic that was not well served. | *required* |
| `session_id` | `str \| None` | Optional calling session identifier. | None |
| `details` | `dict[str, Any] \| None` | Optional additional metadata. | None |
...
```python
def report_issue(self, entry_key: str, issue: str, *, session_id: str | None = None, details: dict[str, Any] | None = None) -> FeedbackEvent:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `entry_key` | `str` | The memory entry key that has the quality issue. | *required* |
| `issue` | `str` | Human-readable description of the issue. | *required* |
| `session_id` | `str \| None` | Optional calling session identifier. | None |
| `details` | `dict[str, Any] \| None` | Optional additional metadata. | None |
...
```python
def record_feedback(self, event_type: str, *, entry_key: str | None = None, session_id: str | None = None, utility_score: float | None = None, details: dict[str, Any] | None = None) -> FeedbackEvent:
```
...
(`[`rate_recall`](#rate-recall)`, `[`report_gap`](#report-gap)`, `[`report_issue`](#report-issue)`) for standard
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `event_type` | `str` | Object-Action snake_case event name (open enum). | *required* |
| `entry_key` | `str \| None` | Optional memory entry key this event relates to. | None |
| `session_id` | `str \| None` | Optional calling session identifier. | None |
| `utility_score` | `float \| None` | Numeric utility signal in [-1.0, 1.0]. | None |
| `details` | `dict[str, Any] \| None` | Optional additional metadata. | None |
...
- ValueError: If *event_type* fails pattern validation, or if strict event types are enabled and the type is unknown.
...
```python
def query_feedback(self, *, event_type: str | None = None, entry_key: str | None = None, session_id: str | None = None, since: str | None = None, until: str | None = None, limit: int = 100) -> list[FeedbackEvent]:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `event_type` | `str \| None` | Filter by exact event type (or None for all). | None |
| `entry_key` | `str \| None` | Filter by related memory entry key. | None |
| `session_id` | `str \| None` | Filter by session identifier. | None |
| `since` | `str \| None` | ISO-8601 lower bound (inclusive) on timestamp. | None |
| `until` | `str \| None` | ISO-8601 upper bound (inclusive) on timestamp. | None |
| `limit` | `int` | Maximum number of results (default 100). | 100 |

**Returns:** `list[FeedbackEvent]` - Matching ``FeedbackEvent`` objects ordered by timestamp ascending.
...
# `src.tapps_brain._store_integrity`
...
TAP-2857: the upgrade branch of :meth:[`rehash_integrity_v1`](#rehash-integrity-v1) previously referenced
...
### `IntegrityMixin`(_MemoryStoreBase)
...
**Returns:** `dict[str, Any]` - Dict with ``total``, ``verified``, ``tampered``, ``no_hash``,
...
it, :meth:[`verify_integrity`](#verify-integrity) will validate all entries under the v2
scheme.  The shim is safe to run multiple times — v2 entries are a
no-op.

**Returns:** `dict[str, int]` - Dict with ``upgraded``, ``tampered``, ``skipped_no_hash``,
    ``already_v2`` counts.
...
# `src.tapps_brain._store_query`

Read / query methods for :class:`~tapps_brain.store.MemoryStore` (TAP-2833).

Extracted from ``store.py`` as a mixin.  Covers point reads (`[`get`](#get)`),
filtered listing (`[`list_all`](#list-all)` / `[`list_memory_groups`](#list-memory-groups)`), deletion, the
...
```python
def get(self, key: str, scope: str | None = None, branch: str | None = None) -> MemoryEntry | None:
```
...
```python
def list_all(self, tier: str | None = None, scope: str | None = None, tags: list[str] | None = None, memory_group: str | None = None, include_superseded: bool = True) -> list[MemoryEntry]:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `tier` | `str \| None` | Filter by tier. | None |
| `scope` | `str \| None` | Filter by scope. | None |
| `tags` | `list[str] \| None` | Filter by tags. | None |
| `memory_group` | `str \| None` | When set, only entries in this project-local group (``None`` on entry means ungrouped; omit this arg to list all). | None |
| `include_superseded` | `bool` | When ``False``, exclude temporally invalid (superseded/expired) entries. Default ``True`` for backward compatibility. | True |
...
```python
def search(self, query: str, tags: list[str] | None = None, tier: str | None = None, scope: str | None = None, memory_group: str | None = None, as_of: str | None = None, include_historical: bool = False, since: str | None = None, until: str | None = None, time_field: str = 'created_at', include_group_memories: bool = False, max_group_results: int = 20, memory_class: str | None = None) -> list[MemoryEntry]:
```
...
| `tier` | `str \| None` | Filter by tier. | None |
| `scope` | `str \| None` | Filter by scope. | None |
| `memory_group` | `str \| None` | When set, restrict to this project-local group (GitHub #49). | None |
| `as_of` | `str \| None` | ISO-8601 timestamp for point-in-time temporal filtering. When set, only entries valid at that time are returned. When ``None`` (default), temporally invalid entries are excluded using the current time. | None |
| `include_historical` | `bool` | When True, include expired/superseded entries (GitHub #29, task 040.3). When False (default), entries whose ``invalid_at`` or ``valid_until`` is in the past are excluded. | False |
| `since` | `str \| None` | ISO-8601 UTC lower bound (inclusive) on *time_field* (Issue #70). | None |
| `until` | `str \| None` | ISO-8601 UTC upper bound (exclusive) on *time_field* (Issue #70). | None |
| `time_field` | `str` | Column to filter on — ``created_at``, ``updated_at``, or ``last_accessed``. Defaults to ``created_at``. | 'created_at' |
| `include_group_memories` | `bool` | When True and the store has declared groups, also search group namespaces in the Hive (STORY-056.5). | False |
| `max_group_results` | `int` | Maximum results per group namespace (STORY-056.5). | 20 |
| `memory_class` | `str \| None` | TAP-733 — when set, restrict to entries with this semantic class (``"incident"``, ``"guidance"``, ``"decision"``, ``"convention"``). Pushed into the SQL WHERE clause for DB-level pre-filtering. | None |
...
# `src.tapps_brain._store_relations`
...
### `RelationsMixin`(_MemoryStoreBase)
...
list of relation dicts (same shape as :meth:[`get_relations`](#get-relations)).
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `key` | `str` | The memory entry key. | *required* |

**Returns:** `list[dict[str, Any]]` - List of relation dicts with subject, predicate, object_entity,
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `keys` | `list[str]` | Memory entry keys to look up. | *required* |

**Returns:** `dict[str, list[dict[str, Any]]]` - Dict mapping each requested key to its list of relation dicts.
...
**Returns:** `Iterator[MemoryEntry]`

#### `count_orphaned_relations`
...
persistence layer by :meth:[`save_relations`](#save-relations) / :meth:[`load_relations`](#load-relations))
...
**Returns:** `int` - Number of ``source_entry_keys`` references in any cached relation
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `now` | `datetime \| None` | Reference timestamp (UTC).  Defaults to ``datetime.now(UTC)`` when *None*. | None |

**Returns:** `int` - Number of entries whose ``valid_at`` field is non-*None* and
    falls before *now*.
...
```python
def find_related(self, key: str, *, max_hops: int = 2) -> list[tuple[str, int]]:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `key` | `str` | Starting entry key. | *required* |
| `max_hops` | `int` | Maximum traversal depth (default 2). | 2 |

**Returns:** `list[tuple[str, int]]` - List of ``(entry_key, hop_distance)`` tuples, ordered by hop
...
```python
def query_relations(self, *, subject: str | None = None, predicate: str | None = None, object_entity: str | None = None) -> list[dict[str, Any]]:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `subject` | `str \| None` | Filter by subject entity. | None |
| `predicate` | `str \| None` | Filter by predicate/relationship type. | None |
| `object_entity` | `str \| None` | Filter by object entity. | None |

**Returns:** `list[dict[str, Any]]` - List of matching relation dicts.
...
```python
def propagate(*, key: str, value: str, agent_scope: str, agent_id: str, agent_profile: str, tier: str, confidence: float, source: str, tags: list[str] | None, hive_store: HiveBackend, auto_propagate_tiers: list[str] | None = None, private_tiers: list[str] | None = None, bypass_profile_hive_rules: bool = False, dry_run: bool = False, memory_group: str | None = None, embedding: list[float] | None = None) -> dict[str, Any]:
```
...
```python
def docs_import_dir_cmd(cache_dir: Annotated[Path, typer.Argument(help='Path to legacy .tapps-mcp-cache directory.')], skip_existing: Annotated[bool, typer.Option(help='Skip library/topic keys already present in brain.')] = True, project_dir: ProjectDir = None, as_json: JsonFlag = False) -> None:
```
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `cache_dir` | `Annotated[Path, typer.Argument(help='Path to legacy .tapps-mcp-cache directory.')]` |  | *required* |
| `skip_existing` | `Annotated[bool, typer.Option(help='Skip library/topic keys already present in brain.')]` |  | True |
| `project_dir` | `ProjectDir` |  | None |
| `as_json` | `JsonFlag` |  | False |
...
### `maintenance_hnsw_reindex`

```python
def maintenance_hnsw_reindex(dsn: Annotated[str, typer.Option('--dsn', envvar='TAPPS_BRAIN_DATABASE_URL', help='PostgreSQL DSN (or set TAPPS_BRAIN_DATABASE_URL).')] = '', dry_run: Annotated[bool, typer.Option('--dry-run', help='Report what would run without issuing SQL.')] = False, as_json: JsonFlag = False) -> None:
```
...
| `dry_run` | `Annotated[bool, typer.Option('--dry-run', help='Report what would run without issuing SQL.')]` |  | False |
| `as_json` | `JsonFlag` |  | False |
...
*Documentation coverage: 100.0%*
*Quality: 9 functions missing examples*
...
```python
def __init__(self, api_key: str | None, *, base_url: str = CONTEXT7_BASE_URL, timeout: float = 10.0) -> None:
```
...
```python
def fetch_docs(self, library_id: str, *, topic: str = 'overview', mode: str = 'code', max_tokens: int = 5000) -> str:
```
...
```python
def import_cache_dir(store: Any, cache_dir: Path, *, config: DocsConfig | None = None, skip_existing: bool = True) -> ImportDirReport:
```
...
```python
def open_docs_store(cfg: DocsConfig | None = None, *, project_root: Path | None = None) -> MemoryStore:
```

Open a ``MemoryStore`` for the shared library-docs project (ADR-0014).

Doc cache rows live under ``(project_id, agent_id)`` from :class:[`DocsConfig`](#docsconfig),
not the MCP caller's project.  Callers must ``close()`` when done unless using
:func:[`get_docs_store`](#get-docs-store) (process-wide cache for MCP tools).
...
```python
def docs_lookup(store: Any, *, library: str, topic: str = 'overview', mode: str = 'code', config: DocsConfig | None = None) -> dict[str, Any]:
```
...
```python
def docs_warm(store: Any, libraries: list[str], *, topic: str = 'overview', mode: str = 'code', config: DocsConfig | None = None) -> dict[str, Any]:
```
...
| Name | Type | Value |
|------|------|-------|
| `DEFAULT_DOCS_CACHE_TTL_SECONDS` | `` | `86400.0` |
...
```python
def embedding_startup_status(provider: SentenceTransformerProvider | None) -> dict[str, Any]:
```

Summarize embedding-provider state for startup logs / health probes.

Returns ``enabled`` (provider loaded), `[`model_id`](#model-id)`, and `[`dimension`](#dimension)` so a
...
| Name | Type | Description | Default |
|------|------|-------------|---------|
| `provider` | `SentenceTransformerProvider \| None` |  | *required* |

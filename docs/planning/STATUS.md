...
**Last updated:** 2026-04-02 (America/Chicago) — **v2.0.3**; **EPIC-041** done; **#51**–**#64** closed; **EPIC-042** stories **042.1–042.8** done (epic success criteria / eval note still in `EPIC-042.md`); **EPIC-050** partial (050.3 + WAL checkpoint runbook); **EPIC-044** **044.1**–**044.2** RAG safety + Bloom dedup, **044.3** save-path conflicts (NLI backlog), **044.4** audit + `run_consolidation_threshold_sweep` + CLI `maintenance consolidation-threshold-sweep` (undo backlog), **044.5** GC dry-run/metrics/`archive.jsonl`, **044.6** `seeding.seed_version` + `profile_seed_version` on health/stats/native health, **044.7** eviction docs (per-group cap backlog); **next-session handoff:** [`next-session-prompt.md`](next-session-prompt.md)
...
1. **EPIC-044** — backlog: **044.3** NLI/async research, **044.4** merge **undo**, **044.7** optional per-group caps; **044.1**–**044.7** core themes shipped (consolidation threshold sweep, GC metrics/dry-run, seed version, eviction docs).
2. **EPIC-050** — lock-scope reduction still deferred unless benchmark-driven; async wrapper (`tapps_brain_async`) optional spike only.
3. **EPIC-032** — OTel GenAI semantic conventions (deferred).
4. Optional: **EPIC-042** epic-level eval/benchmark evidence + GitHub hygiene per `EPIC-042.md` success criteria.

...
- [`system-architecture.md`](../../engineering/system-architecture.md) **Concurrency model** — store serialization, lock ordering, optional `TAPPS_STORE_LOCK_TIMEOUT_S`, Postgres connection pool tuning (`TAPPS_BRAIN_PG_POOL_MAX`), operator triage in the operator runbook and [`postgres-backup-restore.md`](../../operations/postgres-backup-runbook.md) *(sqlite-database-locked.md removed — SQLite retired in ADR-007)*.

...
1. **Shipped / maintained path (do):** Stay **embedded SQLite–first**:
   ...
   - **Dense (optional):** `[vector]` extra — embeddings in `embeddings.py`, vectors on entries and in **`sqlite-vec`** (`memory_vec`) when the extension is available; optional **FAISS** for in-process index use cases already supported by the extra.
...
- **`[vector]` extra:** Continues to mean *local / embedded* dense retrieval helpers (sentence-transformers, numpy, optional faiss, sqlite-vec binding) — not a managed service client.

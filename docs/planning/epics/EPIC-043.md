...
- [x] Schema changes remain **migratable** from v17 with tests.
- [x] Security-sensitive paths (encryption, audit) have **operator docs** updates when behavior changes.
...
- [x] Audit **PRAGMA** set on open; document in engineering doc.
- [x] Metrics: **sqlite busy** / lock wait (if observable).
- [x] Spike: read-only **connection** for heavy search paths vs single shared connection (if safe with our lock model).
...
- [x] Migration **dry-run** or `EXPLAIN` logging in debug mode.
- [x] Automated test: **fresh DB** + **upgrade from fixture vN** snapshot.
...
- [x] Document tokenizer + **rebuild** command if tokenizer ever changes (major version bump).
- [x] Consistency test: insert → immediate FTS search finds row.
...
- [x] Pin **upper bound** policy in `pyproject.toml` with rationale.
- [x] Generate **JSON Schema** for profile subset for editor tooling (optional).
...
- [x] Standardize **event names** for save/recall/error (`memory_save_*`, `recall_*`).
- [x] Document **log levels** for operators.
...
- [x] Runbook: **lost key = data loss** prominently.
- [x] Spike: **HSM/KMS** integration story (out of process key) for enterprise.
- [x] Automated test matrix note: CI may skip without native SQLCipher.
...
- [x] Document **fields** and **rotation** recommendation.
- [x] Optional: **async batch** writer to reduce fsync hot spots (careful with durability).

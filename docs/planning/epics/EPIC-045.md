...
- [x] No regression to **scope semantics** documented in `memory-scopes.md` / `hive-vs-federation.md`.
...
- [x] **Quota** and **rate** limits on Hive writes (profile or server-wide).
- [x] **Backup/restore** Hive DB operator section.
- [x] Observability: **propagate denied** metrics by reason (`hive.propagate.group_denied`, etc.).
...
- [x] **Filter DSL** for publish (deterministic parser).
- [x] **Conflict** story when subscriber edits federated rows locally.
...
- [x] Doc diagram: **one query → namespace list** resolution order.
- [x] Fuzz tests for **normalization** (`group:` edge cases).
...
- [x] **Default group** in profile for MCP/CLI saves (opt-in).
- [x] **list_groups** performance on huge stores (index audit).
...
- [x] **Backoff** policy documented for `hive_wait_write`.
- [x] Spike: **inotify**/ReadDirectoryChangesW integration doc for Windows vs Unix.

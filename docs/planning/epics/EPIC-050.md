...
- **WAL checkpoint** for long-lived MCP — operator guidance in `sqlite-database-locked.md` *(guide removed — SQLite retired in ADR-007)* § *WAL checkpoint* and the operator runbook § *Long-lived MCP and SQLite WAL* (2026-04-02).
...
- [x] Operator **WAL checkpoint** note (long-lived MCP): `sqlite-database-locked.md` + the operator runbook with links to SQLite pragma / WAL docs (2026-04-02).

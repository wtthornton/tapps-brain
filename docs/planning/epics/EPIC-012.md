...
- [ ] ContextEngine plugin implements `bootstrap`, `ingest`, `afterTurn`, `compact` hooks
- [ ] Auto-recall injects relevant memories before each turn (no explicit tool call needed)
- [ ] Auto-capture extracts facts from agent responses after each turn
- [ ] Pre-compaction flush saves important context before OpenClaw compresses the context window
- [ ] MEMORY.md import migrates existing OpenClaw memories into tapps-brain on first run
- [ ] ClawHub skill published with one-command install
- [ ] PyPI package published and installable via `pip install tapps-brain[mcp]`
...
- [ ] New `src/tapps_brain/markdown_import.py` module
- [ ] `import_memory_md(path: Path, store: MemoryStore) -> int` — parses MEMORY.md, creates entries, returns count imported
- [ ] Parses markdown structure: headings become keys (slugified), body becomes value
- [ ] Detects and imports `memory/YYYY-MM-DD.md` daily note files as `context`-tier entries with appropriate dates
- [ ] Deduplication: skips entries whose keys already exist in the store
- [ ] Tier inference: heading-level heuristic — H1/H2 → `architectural`, H3 → `pattern`, H4+ → `procedural`, daily notes → `context`
- [ ] `import_openclaw_workspace(workspace_dir: Path, store: MemoryStore) -> dict` — imports both MEMORY.md and memory/*.md, returns counts
- [ ] Unit test: import a sample MEMORY.md, verify entries created with correct tiers
- [ ] Unit test: import twice, verify no duplicates
- [ ] Unit test: import daily notes with date extraction
...
- [ ] New `openclaw-plugin/` directory at repo root with plugin structure
- [ ] `openclaw-plugin/plugin.json` — plugin manifest with ContextEngine slot registration
- [ ] `openclaw-plugin/src/index.ts` — TypeScript entry point that spawns `tapps-brain-mcp` as a child process
- [ ] `bootstrap` hook: opens MemoryStore, imports MEMORY.md if first run, runs initial `recall()` for session primer
- [ ] Plugin reads `--project-dir` from OpenClaw workspace path
- [ ] Plugin respects profile from `{workspace}/.tapps-brain/profile.yaml`
- [ ] README with install instructions: `openclaw plugin install ./openclaw-plugin`
- [ ] Manual test: install plugin in local OpenClaw, verify it loads and bootstrap runs
...
- [ ] `ingest` hook receives the user's message text
- [ ] Calls `memory_recall(message)` via MCP to get ranked memories
- [ ] Injects the `memory_section` into the context window as a system-level prefix
- [ ] Respects token budget from profile (`recall.default_token_budget`)
- [ ] Skips injection if no relevant memories found (empty `memory_section`)
- [ ] Deduplication: tracks keys already injected in this session to avoid repeating them
- [ ] Latency: recall + injection completes in <500ms for typical stores (<500 entries)
- [ ] Manual test: ask OpenClaw about something previously stored, verify it recalls without explicit tool call
...
- [ ] `afterTurn` hook receives the agent's response text
- [ ] Calls `memory_capture(response)` via MCP to extract and persist facts
- [ ] Only captures if response contains decision-like statements (delegates to existing extraction logic)
- [ ] Rate-limited: captures at most once every 3 turns to avoid flooding the store
- [ ] Logs captured keys for observability
- [ ] Manual test: have OpenClaw make a decision ("let's use Redis for caching"), verify it's captured as a memory entry
...
- [ ] `compact` hook receives the full conversation context about to be compacted
- [ ] Calls `memory_ingest(context)` via MCP to extract durable facts
- [ ] Extracts session summary and indexes it via `memory_index_session(session_id, chunks)`
- [ ] Session ID derived from OpenClaw session/conversation identifier
- [ ] Only extracts from the portion of context being compacted (not already-persisted memories)
- [ ] Manual test: have a long OpenClaw conversation, verify memories are saved when compaction triggers
...
- [ ] `pyproject.toml` updated with `project.urls` (homepage, repository, documentation, changelog)
- [ ] `uv build` produces clean wheel and sdist
- [ ] Published to PyPI: `pip install tapps-brain[mcp]` works
- [ ] `openclaw-skill/` directory with `SKILL.md` (YAML frontmatter + instructions) and `openclaw.plugin.json`
- [ ] SKILL.md declares all 21 MCP tools, triggers, capabilities, permissions
- [ ] `openclaw.plugin.json` auto-configures MCP server on install
- [ ] Submitted to ClawHub registry (PR to `github.com/openclaw/clawhub` or `openclaw skill publish`)
- [ ] Version consistency check: pyproject.toml version = SKILL.md version = plugin.json version
- [ ] End-to-end test: fresh OpenClaw install → `openclaw skill install tapps-brain-memory` → "remember X" → "recall X" works
...
- [ ] Integration test: Markdown import — create a mock MEMORY.md with headings and content, import into store, verify entries with correct tiers
- [ ] Integration test: Markdown import idempotency — import twice, verify no duplicates
- [ ] Integration test: recall + capture round-trip — save a memory, recall it via orchestrator, capture a response containing new facts, verify new entries created
- [ ] `docs/guides/openclaw.md` updated with ContextEngine plugin instructions alongside MCP sidecar instructions
- [ ] Documentation covers: install, bootstrap, auto-recall, auto-capture, pre-compaction, profile switching, Hive integration

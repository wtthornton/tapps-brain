...
- [x] **#23** `feat: SQLCipher support`
  - Status: `done` (`[encryption]` extra / `pysqlcipher3`; `sqlcipher_util` + `encryption_migrate`; `MemoryPersistence` / `FeedbackStore` / `DiagnosticsHistoryStore` / `HiveStore` / `MemoryStore`; CLI `maintenance encrypt-db|decrypt-db|rekey-db`; `StoreHealthReport.sqlcipher_enabled`; `docs/guides/sqlcipher.md` *(guide removed — SQLite retired in ADR-007)*)
...
- 2026-04-03: **EPIC-051 STORY-051.5** — SQLCipher ops [`adr/ADR-005-sqlcipher-key-backup-operations.md`](adr/ADR-005-sqlcipher-key-backup-operations.md); `sqlcipher.md` *(guide removed — SQLite retired in ADR-007)* — key loss, backup/restore checklist, enterprise KMS note; defer vendor envelope how-tos; `features-and-technologies.md` item 5 + section 2.
...
- 2026-04-02 (eve): **EPIC-050** residual — WAL checkpoint guidance for long-lived MCP: `sqlite-database-locked.md` *(guide removed — SQLite retired in ADR-007)* § *WAL checkpoint*; [`openclaw-runbook.md`](../guides/openclaw-runbook.md) § *Long-lived MCP and SQLite WAL*; `EPIC-050.md` research/themes synced.
...
- 2026-04-02 (late): **STORY-042.3** — `sqlite-vec-operators.md` *(guide removed — SQLite retired in ADR-007)* (rebuild/VACUUM, incremental upsert cost, L2 default vs `MATCH` SQL); `sqlite_vec_index.py` / `persistence.py` / `retrieval.py` docstrings; engineering cross-links; sqlite_vec test asserts L2 distance ordering.

...
<!-- flagged: 8 heading(s) duplicate the managed block above verbatim (## Routing guide, ## Shell auth (CLI memory), ## Decide: should I write to memory?, ## Do NOT save, ## Pick a tier (when saving), ## CLI commands (daily drivers), ## Advanced surface, ## See also); 100% of this region's lines duplicate the managed block above — review and trim -->

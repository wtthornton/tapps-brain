...
<!-- flagged: 3 heading(s) duplicate the managed block above verbatim (# on cache hit, use snap.data.issues directly., # on miss: list_issues(team, project, includeArchived=false)  # OMIT state — "open" is not Linear, #          filter to open statusTypes, then snapshot_put(..., state="open")); 81% of this region's lines duplicate the managed block above — review and trim -->

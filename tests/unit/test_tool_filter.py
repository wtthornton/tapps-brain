...
class TestListToolsFilter:
    ...
    def test_bundled_coder_profile_returns_22_tools(self) -> None:
        """Real ProfileRegistry.get('coder') must return exactly 22 tools."""
        real_registry = ProfileRegistry()
        coder_tools = real_registry.get("coder")
        assert len(coder_tools) == 22
...
class TestDeferredToolBehavior:
    ...
    def test_small_profiles_all_eager(self) -> None:
        ...
        expected_counts = {"coder": 22, "reviewer": 10, "seeder": 6, "agent_brain": 23}

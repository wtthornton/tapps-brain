...
class PostgresHiveBackend:
        ...
        *,
        ...
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                created_at = (
                    existing_dict.get("created_at", now)
                    if existing is not None
                    else now
                )
    ...
    def _write_entry(
        ...
        *,
        ...
        cur.execute(
            ...
            (
                namespace, key, value, source_agent, tier, confidence, source,
                tags_json, valid_at, invalid_at, superseded_by, memory_group,
    ...
    def search(
        ...
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                rows = cur.fetchall()
                col_names = [desc[0] for desc in cur.description]
                return [self._row_to_dict(dict(zip(col_names, r))) for r in rows]
    ...
    def add_group_member(
        ...
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT name FROM hive_groups WHERE name = %s", (group_name,)
                )
        ...
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "WHERE agent_id = %s ORDER BY group_name",
                    (agent_id,),
                )
    ...
    def agent_is_group_member(self, group_name: str, agent_id: str) -> bool:
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT 1 FROM hive_group_members "
                    "WHERE group_name = %s AND agent_id = %s",
                    (group_name, agent_id),
                )
        ...
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    """,
                    (
                        event_id, namespace, entry_key, event_type, session_id,
                        utility_score, details_json, timestamp, source_project,
                    ),
                )
    ...
    def query_feedback_events(
        ...
        *,
        ...
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"SELECT * FROM hive_feedback_events {where} "  # noqa: S608
    ...
    def _get_state_from_conn(self, conn: Any) -> dict[str, Any]:
...
class PostgresAgentRegistry:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO agent_registry
                        (id, name, profile, skills, project_root, groups, registered_at, last_seen_at)
    ...
    def unregister(self, agent_id: str) -> bool:
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "DELETE FROM agent_registry WHERE id = %s", (agent_id,)
                )
                return cur.rowcount > 0
    ...
    def get(self, agent_id: str) -> Any | None:
        with self._cm.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT * FROM agent_registry WHERE id = %s", (agent_id,)
                )
                row = cur.fetchone()
                if row is None:
                    return None
